...
const functions = {
    ...
    async load(ev,options) {
        await load.call(this,options);
        return true;
    },
    preventDefault(ev) {
        ev.preventDefault();
    ...
    const keywords = ["once", "capture", "passive", "preventDefault", "stop","stopImmediate"],
        ...
        mods = parts.filter(p => p.includes(":") || ["preventDefault", "stop","stopImmediate"].includes(p)).reduce((mods, mod) => {
                ...
                if (functions[f]) {
                    ...
                    mods[f] = function(ev) { return functions[f].call(this,ev,arg); }
                    mods[f].arg = arg;
                } else {
...
xon.patchDOM = (root,ctx) => {
    [...root.querySelectorAll('[x-on]')].forEach(el => xon(el,el.getAttribute('x-on'),ctx));
}

export {xon, xon as default}